...
const { ControlledSpecialistDispatcher, IndependentDispatchEvaluator } = require('./dispatch');
const {
  AgentFactory,
  Evaluator,
  EvolutionEngine,
...
class MotherEvolutionLoop {
  constructor(projectRoot, options = {}) {
    ...
    this.dispatcher = options.dispatcher || new ControlledSpecialistDispatcher(this.sandbox);
    this.dispatchEvaluator = options.dispatchEvaluator || new IndependentDispatchEvaluator();
  }
  ...
  run(options) {
    ...
    const activeCandidates = architecture.blueprints.map(blueprint => {
      const active = this.registry.getActive(blueprint.primaryCapability);
      if (!active?.definition) throw new Error(`No active specialist for ${blueprint.primaryCapability} after evolution.`);
    ...
    const dispatchEvaluation = this.dispatchEvaluator.evaluate({ dispatch, routePlan, gaps: coverageMap.gaps });
    this.registry.appendAudit('specialists.dispatched', {
      runId,
      dispatchDigest: dispatch.digest,
      passed: dispatchEvaluation.passed,
      assignmentCount: dispatch.assignments.length,
    });
    if (!dispatchEvaluation.passed) {
      this.registry.appendAudit('specialists.dispatch-rejected', { runId, checks: dispatchEvaluation.checks });
      this.registry.save();
      throw new Error('Independent specialist dispatch evaluation failed.');
    }
